...
const buildTextCanvas = ({
  ...
  const fontWeight =
    computed.fontWeight || String(props.fontWeight);
  let letterSpacing =
  ...
  const widest = Math.max(
    ...lines.map((line) =>
      measureLine(ctx, line, letterSpacing),
    ),
    1,
  );
  ...
  useEffect(() => {
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTime: { value: 0 },
        uWarpStrength: {
          value: propsRef.current.warpStrength,
        },
        uWarpScale: { value: propsRef.current.warpScale },
        uSpeed: { value: propsRef.current.speed },
        uPointerInfluence: {
          value: propsRef.current.pointerInfluence,
        },
        uPointerStrength: {
          value: propsRef.current.pointerStrength,
        },
    ...
    const loop = (now) => {
      ...
      const targetX =
        pointer.activeTarget > 0 ? pointer.tx : idleX;
      const targetY =
        pointer.activeTarget > 0 ? pointer.ty : idleY;
      const damping =
        pointer.activeTarget > 0 ? 0.12 : 0.035;

      pointer.x += (targetX - pointer.x) * damping;
      pointer.y += (targetY - pointer.y) * damping;
      pointer.active +=
        ((pointer.activeTarget > 0 ? 1 : 0.18) -
          pointer.active) *
        0.06;
      ...
      program.uniforms.uPointerActive.value = reduceMotion
        ? pointer.active * 0.35
        : pointer.active;
      program.uniforms.uTime.value = reduceMotion
        ? 0
        : elapsed;
